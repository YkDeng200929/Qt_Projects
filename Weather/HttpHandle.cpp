...
{
    QNetworkRequest request(QUrl(QString("https://geoapi.qweather.com/"
                                         "v2/city/lookup?location=%1&key=%2").arg(city).arg(weather_api)));
...
{
    QNetworkRequest request(QUrl(QString("https://devapi.qweather.com/"
                                         "v7/weather/3d?location=%1&key=%2").arg(cityId).arg(weather_api)));
    QNetworkReply *reply = manager->get(request);
...
void HttpHandle::searchCityWeather(const QString &cityId, const QString &cityName,
                                   const QString &admain)
{
